...
export async function POST(req:Request,{params}:{params:Promise<{tripId:string}>}){
 ...
 const next=resetTripCopy(state,title,startDate,endDate);let copyId=slugForTrip({title,startDate});let suffix=2;while(await loadState(copyId))copyId=`${slugForTrip({title,startDate})}-${suffix++}`;await saveState(next,copyId);return NextResponse.json({ok:true,tripId:copyId});
}
