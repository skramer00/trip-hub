...
function typeFromText(text:string):ItineraryItemType{const lower=text.toLowerCase();if(/\b(?:flight|airline|airport|train|rail|bus|ferry|depart|departure|arrival|arrive|transport)\b/.test(lower))return'travel';if(/\b(?:hotel|inn|resort|lodging|airbnb)\b|check[- ]?in/.test(lower))return'hotel';if(/\b(?:restaurant|dinner|lunch|breakfast|brunch|table)\b/.test(lower))return'food';if(/\b(?:reservation|ticket|tour|game|show|museum|admission|booking)\b/.test(lower))return'reservation';return'activity';}
