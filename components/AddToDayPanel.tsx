...
import {addToDayPlacementOptions,categoryForGooglePlace,defaultDurationForCategory,formatTripTime,inputTimeValue,regionForTripDay,suggestedAddTime} from '@/lib/add-to-day';
...
type Props={
 ...
 onAddSaved:(place:Place,dayIndex:number,time:string,optional:boolean,travelMode:TravelMode,travelMinutes?:number,insertionIndex?:number)=>void;
 onAddGoogle:(candidate:GooglePlaceCandidate,dayIndex:number,time:string,optional:boolean,travelMode:TravelMode,travelMinutes?:number,insertionIndex?:number)=>void;
 onAddCustom:(value:{title:string;destination:string;type:ItineraryItemType;dayIndex:number;time:string;optional:boolean;travelMode:TravelMode;travelMinutes?:number;insertionIndex?:number})=>void;
};
...
function DayPlacementPreview({day,insertionIndex,candidateName,candidateTime,recommended}:{day:TripState['days'][number];insertionIndex:number;candidateName:string;candidateTime:string;recommended:boolean}){
 const rows=[...day.items.map(item=>({id:item.id,time:item.time,title:item.title,fixed:Boolean(item.fixed),candidate:false}))];
 rows.splice(Math.max(0,Math.min(insertionIndex,rows.length)),0,{id:'candidate-stop',time:candidateTime,title:candidateName,fixed:false,candidate:true});
 return <section className="addDayTimeline" aria-label="Day placement preview">
  <div className="addDayTimelineHeading"><span>Day timeline</span><strong>{day.label} · {day.city}</strong></div>
  <ol>{rows.map(row=><li className={row.candidate?'candidate':''} key={row.id}><time>{row.time||'Flexible'}</time><span>{row.title}</span>{row.candidate?<em>{recommended?'Recommended spot':'Selected spot'}</em>:row.fixed?<em>Fixed</em>:null}</li>)}</ol>
 </section>;
}
...
export default function AddToDayPanel({days,places,initialDayIndex,onClose,onAddSaved,onAddGoogle,onAddCustom}:Props){
 ...
 const [placement,setPlacement]=useState<'recommended'|number>('recommended');
 ...
 const placementOptions=useMemo(()=>previewPlace?addToDayPlacementOptions(days[dayIndex],places,previewPlace,tripTime,travelMode,previewDuration):[],[dayIndex,days,places,previewDuration,previewPlace,travelMode,tripTime]);
 const recommendedPlacement=placementOptions.find(option=>option.recommended)??placementOptions[0];
 const selectedPlacement=placement==='recommended'?recommendedPlacement:placementOptions.find(option=>option.index===placement)??recommendedPlacement;
 const preview=selectedPlacement?.preview??null;
 const recommendedPlanApplied=placement==='recommended'&&Boolean(preview)&&time===inputTimeValue(recommendedPlacement?.preview.suggestedTime??'');
 function applyRecommendedPlan(){
  if(!recommendedPlacement)return;
  setPlacement('recommended');
  setTime(inputTimeValue(recommendedPlacement.preview.suggestedTime));
 }
 function selectDay(index:number){setDayIndex(index);setTime(inputTimeValue(suggestedAddTime(days[index])));setSelected(null);setPlacement('recommended');}
 function chooseSaved(place:Place){setSelected({source:'saved',place});setPlacement('recommended');}
 ...
 function confirmSelected(){
  ...
  if(selected.source==='saved')onAddSaved(selected.place,dayIndex,tripTime,optional,travelMode,travelMinutes,selectedPlacement?.index);
  else onAddGoogle(selected.candidate,dayIndex,tripTime,optional,travelMode,travelMinutes,selectedPlacement?.index);
  onClose();
 }
 ...
 return <div className="editorUnlockBackdrop" role="presentation" onMouseDown={event=>{if(event.currentTarget===event.target)onClose();}}>
  <section className="card addToDayPanel" role="dialog" aria-modal="true" aria-labelledby="add-to-day-title">
   ...
   <div className="viewSwitch addSourceSwitch" aria-label="Stop source"><button className={mode==='saved'?'active':''} onClick={()=>{setMode('saved');setSelected(null);setPlacement('recommended');}}>Saved places</button><button className={mode==='google'?'active':''} onClick={()=>{setMode('google');setSelected(null);setPlacement('recommended');}}>Search Google</button><button className={mode==='custom'?'active':''} onClick={()=>{setMode('custom');setSelected(null);setPlacement('recommended');}}>Custom stop</button></div>
   ...
   {selected&&preview&&mode!=='custom'&&<div className="selectedAddStop"><div className="selectedAddStopHeading"><div><span>Selected stop</span><strong>{selected.place.name}</strong></div><button className="textButton" type="button" onClick={()=>setSelected(null)}>Choose another</button></div><label className="addPlacementField">Position<select className="field" aria-label="Stop position" value={placement} onChange={event=>setPlacement(event.target.value==='recommended'?'recommended':Number(event.target.value))}><option value="recommended">Recommended · {recommendedPlacement?.label}</option>{placementOptions.map(option=><option value={option.index} key={option.index}>{option.label}{option.recommended?' · best route':''}</option>)}</select></label><button className="btn addRecommendedPlan" type="button" disabled={recommendedPlanApplied} onClick={applyRecommendedPlan}>{recommendedPlanApplied?'Recommended position and time applied':'Use recommended position + time'}</button><DayPlacementPreview day={days[dayIndex]} insertionIndex={selectedPlacement?.index??days[dayIndex].items.length} candidateName={selected.place.name} candidateTime={tripTime} recommended={placement==='recommended'}/><RoutePreview preview={preview} travelMode={travelMode} onTravelMode={setTravelMode} onUseSuggested={()=>setTime(inputTimeValue(preview.suggestedTime))}/><button className="btn primary addConfirm" type="button" onClick={confirmSelected}>{selected.source==='google'?'Save place + add to day':'Add to day'}</button></div>}
   ...
   {mode==='custom'&&<div className="customStopForm"><label>Stop name<input autoFocus className="field" value={title} onChange={event=>setTitle(event.target.value)} placeholder="Lunch, scenic walk, hotel break…"/></label><label>Destination or address<input className="field" value={destination} onChange={event=>setDestination(event.target.value)} placeholder="Optional, but needed for directions"/></label><label>Type<select className="field" value={type} onChange={event=>setType(event.target.value as ItineraryItemType)}><option value="activity">Activity</option><option value="food">Food</option><option value="travel">Travel</option><option value="hotel">Hotel</option><option value="reservation">Reservation</option></select></label>{preview&&<div className="customRoutePreview"><label className="addPlacementField">Position<select className="field" aria-label="Stop position" value={placement} onChange={event=>setPlacement(event.target.value==='recommended'?'recommended':Number(event.target.value))}><option value="recommended">Recommended · {recommendedPlacement?.label}</option>{placementOptions.map(option=><option value={option.index} key={option.index}>{option.label}{option.recommended?' · best route':''}</option>)}</select></label><button className="btn addRecommendedPlan" type="button" disabled={recommendedPlanApplied} onClick={applyRecommendedPlan}>{recommendedPlanApplied?'Recommended position and time applied':'Use recommended position + time'}</button><DayPlacementPreview day={days[dayIndex]} insertionIndex={selectedPlacement?.index??days[dayIndex].items.length} candidateName={title.trim()} candidateTime={tripTime} recommended={placement==='recommended'}/><RoutePreview preview={preview} travelMode={travelMode} onTravelMode={setTravelMode} onUseSuggested={()=>setTime(inputTimeValue(preview.suggestedTime))}/></div>}<button className="btn primary" disabled={!title.trim()} onClick={()=>{onAddCustom({title:title.trim(),destination:destination.trim(),type,dayIndex,time:tripTime,optional,travelMode,travelMinutes:preview?.incoming?.minutes,insertionIndex:selectedPlacement?.index});onClose();}}>Add custom stop</button></div>}
  </section>
 </div>;
}
