...
export default function TripApp(){
 ...
 function addItem(di:number,value?:{title:string;destination:string;type:ItineraryItem['type'];time:string;optional:boolean;travelMode?:TravelMode;travelMinutes?:number;insertionIndex?:number}){if(!state)return;const next=structuredClone(state);const destination=value?.destination??'';const item:ItineraryItem={id:`custom-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,time:value?.time??'12:00 PM',title:value?.title??'New stop',details:'',destination,mapUrl:mapsUrl(destination),routeText:destination?'Open directions from the previous stop.':'',keyInfo:'',userNotes:'',done:false,optional:value?.optional??false,fixed:value?.type==='reservation',type:value?.type??'activity',estimatedDuration:60,travelMinutes:value?.travelMinutes??20,travelMode:value?.travelMode??'transit',prepBuffer:15};if(value?.insertionIndex===undefined){next.days[di].items.push(item);next.days[di].items=sortItems(next.days[di].items);}else next.days[di].items.splice(Math.max(0,Math.min(value.insertionIndex,next.days[di].items.length)),0,item);setBoardUndo(structuredClone(state));void persist(next);}
 function addPlaceToItinerary(place:Place,di:number,optional=false,time='Flexible',travelMode:TravelMode='transit',travelMinutes=20,insertionIndex?:number){
  if(!state)return;
  const next=structuredClone(state);
  const item=itineraryItemFromPlace(place,time,optional,travelMode,travelMinutes);
  if(insertionIndex===undefined){next.days[di].items.push(item);next.days[di].items=sortItems(next.days[di].items);}else next.days[di].items.splice(Math.max(0,Math.min(insertionIndex,next.days[di].items.length)),0,item);
  setBoardUndo(structuredClone(state));
  void persist(next);
 }
 function addGooglePlaceToItinerary(candidate:GooglePlaceCandidate,di:number,time:string,optional:boolean,travelMode:TravelMode='transit',travelMinutes=20,insertionIndex?:number){
  if(!state)return;
  const next=structuredClone(state);
 ...
 return <>
  ...
  {editorView&&addToDayIndex!==null&&<AddToDayPanel days={state.days} places={state.places} initialDayIndex={addToDayIndex} onClose={()=>setAddToDayIndex(null)} onAddSaved={(place,dayIndex,time,optional,travelMode,travelMinutes,insertionIndex)=>addPlaceToItinerary(place,dayIndex,optional,time,travelMode,travelMinutes,insertionIndex)} onAddGoogle={addGooglePlaceToItinerary} onAddCustom={value=>addItem(value.dayIndex,value)}/>}
