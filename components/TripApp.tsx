...
export default function TripApp(){
 ...
 return <>
  ...
  <main className="shell">
   ...
   {tab==='Itinerary'&&<section><div className="pageIntro"><div><div className="eyebrow">FULL SCHEDULE</div><h2>Eight days, one clear plan</h2></div><span className="chip">{completedTrip}/{tripProgress.length} complete</span></div>{state.days.map((day,di)=><article className="card dayCard" key={day.date}><div className="between dayHeader"><div><div className="eyebrow">{day.date}</div><h2>{day.label} · {day.city}</h2></div><span className="chip">{day.items.filter(i=>i.done).length}/{day.items.length}</span></div>{day.items.map((item,ii)=><label className={`itineraryRow ${item.done?'done':''}`} key={item.id}><input type="checkbox" checked={item.done} onChange={()=>toggleDay(di,ii)}/><div className="itineraryTime">{item.time}</div><div><div className="titleRow"><strong>{item.title}</strong>{item.optional&&<span className="chip neutral">Optional</span>}</div>{item.details&&<p className="muted small">{item.details}</p>}</div></label>)}</article>)}</section>}
