...
export default function TripApp(){
 ...
 function editItem(di:number,ii:number,key:'confirmationNumber'|'userNotes',value:string){if(!state)return;const next=structuredClone(state);next.days[di].items[ii][key]=value;setState(next);localStorage.setItem('trip-state',JSON.stringify(next));}
 ...
 return <>
  ...
  <main className="shell">
   ...
   {tab==='Today'&&currentDay&&<section>
    <div className="todayHero card"><div><div className="eyebrow">NEXT UP</div><h2>{currentDay.label} · {currentDay.city}</h2><p className="muted">Recommendations below are selected for this specific itinerary day.</p></div><div className="progressRing" aria-label={`${completedToday} of ${totalToday} complete`}><strong>{completedToday}/{totalToday}</strong><span>done</span></div></div>
...
function ItineraryDetails({item,dayIndex,itemIndex,onEdit,onSave}:{item:ItineraryItem;dayIndex:number;itemIndex:number;onEdit:(di:number,ii:number,key:'confirmationNumber'|'userNotes',value:string)=>void;onSave:()=>void}){
 return <div className="timelineCopy"><div className="titleRow"><h3>{item.title}</h3>{item.optional&&<span className="chip neutral">Optional</span>}</div>{item.details&&<p className="muted small">{item.details}</p>}{item.routeText&&<p className="muted small">🚌 {item.routeText}</p>}{item.mapUrl&&<a className="textLink" href={item.mapUrl} target="_blank" rel="noreferrer">Transit from current location ↗</a>}<div className="filterGrid" style={{marginTop:'12px'}}><label className="small">Confirmation number<input className="field" value={item.confirmationNumber??''} placeholder="Flight, hotel, ticket…" onChange={e=>onEdit(dayIndex,itemIndex,'confirmationNumber',e.target.value)} onBlur={onSave}/></label><label className="small">Personal notes<textarea className="field" rows={2} value={item.userNotes??''} placeholder="Add reminders or details" onChange={e=>onEdit(dayIndex,itemIndex,'userNotes',e.target.value)} onBlur={onSave}/></label></div></div>;
}
