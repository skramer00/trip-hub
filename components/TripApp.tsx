...
type EditableKey='time'|'title'|'details'|'destination'|'routeText'|'keyInfo'|'userNotes'|'optional';
...
export default function TripApp(){
 ...
 function editItem(di:number,ii:number,key:EditableKey,value:string|boolean){if(!state)return;const next=structuredClone(state);const item=next.days[di].items[ii];if(key==='optional')item.optional=Boolean(value);else item[key]=String(value);if(key==='destination')item.mapUrl=mapsUrl(String(value));setState(next);localStorage.setItem('trip-state',JSON.stringify(next));}
 ...
 function addItem(di:number){if(!state)return;const next=structuredClone(state);next.days[di].items.push({id:`custom-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,time:'12:00 PM',title:'New stop',details:'',destination:'',routeText:'',keyInfo:'',userNotes:'',done:false,optional:false});next.days[di].items=sortItems(next.days[di].items);void persist(next);}
